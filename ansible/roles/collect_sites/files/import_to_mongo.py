...
def main():
    ...
    parser.add_argument('--servers_config_wildcard', type=str, default='servers/*.json')
    ...
    parser.add_argument('--replicaSet', default="lethe")
    ...
    #         with codecs.open(config, 'r', 'utf-8-sig') as handler:
    #             configuration = json.load(handler)
    #             configuration['server_name'] = server_name
    #             try:
    #                 configuration['ip'] = socket.gethostbyname(server_name)
    #             except socket.gaierror:
    #                 configuration['ip'] = 'unresolved'
    #                 logger.info("Unable to resolve hostname: %s", server_name)

    #             collection.replace_one({"server_name": server_name}, configuration, upsert=True)
